...
class Keyboard:
    def __init__(self, file_to_safe: str) -> None:
        self.state_saver = KeybordStateSaver(self, file_to_safe, KeyboardSerializer())
        self.undo_stack = []
        self.redo_stack = []
        self.printed_sq = ""
        self.commands = {}
...
# ==========================================
# Тесты с наглядным выводом
# ==========================================

# --- Тест 1: Печать символов и undo/redo ---
print("\n========== Test 1: Print and Undo/Redo ==========")
...
log_command("k.undo()")
k.undo()   # Ожидаем: ab (отмена 'c')

log_command("k.undo()")
k.undo()   # Ожидаем: a (отмена 'b')

log_command("k.redo()")
k.redo()   # Ожидаем: ab (возврат 'b')


# --- Тест 2: Команды громкости ---
print("\n========== Test 2: Volume Commands ==========")

log_command("k.do('ctrl++')")
k.do('ctrl++')  # Ожидаем: volume increased +20%

log_command("k.undo()")
k.undo()        # Ожидаем: volume decreased -20%

log_command("k.do('ctrl+-')")
k.do('ctrl+-')  # Ожидаем: volume decreased -20%

log_command("k.undo()")
k.undo()        # Ожидаем: volume increased +20%


# --- Тест 3: Команда медиаплеера ---
print("\n========== Test 3: Media Player Command ==========")

log_command("k.do('ctrl+p')")
k.do('ctrl+p')  # Ожидаем: media player launched

log_command("k.undo()")
k.undo()        # Ожидаем: media player closed


# --- Тест 4: Сохранение состояния ---
print("\n========== Test 4: Save State ==========")

log_command("k.serialize()")
k.serialize()
print("   [Система]: Состояние сохранено в файл.")


# --- Тест 5: Восстановление состояния ---
print("\n========== Test 5: Load State ==========")

log_command("new_k = Keyboard(TEST_FILE)")
new_k = Keyboard(TEST_FILE)

log_command("new_k.deserialize()")
new_k.deserialize()
print("   [Система]: Состояние загружено.")

print("\n-------------------------------------------")
print("Результат восстановления:")
print(f"Текущая строка (printed_sq): '{new_k.printed_sq}'")  # Должно быть 'ab'
print(f"Стек отмены (undo_stack):    {new_k.undo_stack}")     # История команд
print("-------------------------------------------")